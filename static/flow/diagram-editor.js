...
function displayBlock(block) {
    ...
    if (block.type === 'number_entry') {
        var input = $('<input>', {class: 'form-control flow-block-input', type: 'text', id: 'bv_' + block.id}).appendTo(blockDiv);
        if (block.value !== null) {
...
function appendBlockParametersToBlockDiv(block, blockDiv) {
    for (var i = 0; i < block.params.length; i++) {
        ...
        $('<div>', {class: 'flow-block-param-label', html: param.name}).appendTo(blockDiv);
        var input = $('<input>', {class: 'form-control flow-block-input', type: 'text', id: 'bp_' + param.name, value: param['default']}).appendTo(blockDiv);
        input.mousedown(function(e) {e.stopPropagation()});
...
var CLASS_SCALING_TABLE = {
   ...
   'flow-block-param-label': { "font-size": 13, "line-height": 36, "height": 36, "margin-left": 2 },
   ...
   'flow-block-input-holder-margin': {  "margin-right": 24 },
   'flow-block-input': { "font-size": 14, "margin-top": 3, "margin-right": 2, "margin-bottom": 3, "width": 60, "height": 26 },
   'flow-block-input-long': { "width": 140 },
